		...
		for (const paramData of this.data.TakeParam.concat(this.data.BeginParam)) {
			switch (paramData.Type) {
				...
				case 'MultiSequence':
					...
					requirements.push(`Завершён квест ${mission.plainLink()}`)
				...
				case 'SequenceNextDay':
					...
					requirements.push(`Завершите квест ${gateMission.plainLink()} и дождитесь ежедневного [[Обновление сервера|обновления сервера]].`)
