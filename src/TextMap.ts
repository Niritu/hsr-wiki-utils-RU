...
export class TextMap {
    ...
    static async load(version: Version = config.target_version as Version, lang: SupportedLanguage = 'EN'): Promise<TextMap> {
        ...
        if (['RU', 'TH', 'KR'].includes(lang)) {
            data = {};
            let partIndex = 0;

            while (true) {
                const fileName = `TextMap/TextMap${lang}_${partIndex}.json`;
                try {
                    const partData = await getFile(fileName, version);
                    data = { ...data, ...partData };
                    partIndex++;
                } catch (error: any) {
                    if (error.code === 'ENOENT' || error.message.includes('not found')) break;
            ...
            if (Object.keys(data).length === 0) {
                data = await getFile(`TextMap/TextMap${lang}.json`, version);
            }
        } else {
            data = await getFile(`TextMap/TextMap${lang}.json`, version);
        }

        return new TextMap(version, lang, data)
    }
    ...
    static async preloadOL(version: Version = config.target_version as Version) {
    ...
    static async generateOL(keys?: (string | number | bigint | HashReference) | (string | number | bigint | HashReference | undefined)[], params?: TextParams): Promise<string> {
        ...
        const version = config.target_version as Version
        await this.preloadOL(version)
        
        for (const [i, key] of keys.entries()) {
            if (i != 0) output.push('')
            for (let [tkey, lang] of Object.entries(OTHER_LANGUAGES)) {
                if (keys.length > 1) tkey = `${i+1}_${tkey}`
                output.push(`|${whitespace(tkey, targetWsp)}= ${(await this.load(version, lang)).getText(key, params)}`)
            }
        }
        output.push('}}')
        return output.join('\n')
